    ...
    public List<String> MACtoASE(String[] list) {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            if (command.length() != 4) {
                ...
                code = getOneStr(command).toUpperCase();
                switch (code) {
                    case "1":
                    ...
                    case "A":
                        ...
                        data = getTwoStr(command);
                        reg1 = getThreeStr(command);
                        reg2 = getFourStr(command);

                        if(!data.matches("0")){
                            MachineTools.showMessageDialog(context, "错误行" + (i + 1) + ":指令错误操作数过多");
                            return null;
                        }
                        if(checkRegister(reg1) && checkRegister(reg2)){
                            reslut.add("XCHG R" + reg1 + ",R" + reg2);
                        }else {
                            MachineTools.showMessageDialog(context, "错误行" + (i + 1) + ":寄存器范围0~5");
                            return null;
                        }
